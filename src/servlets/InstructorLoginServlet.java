	...
	protected void service (HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(UserManager.verify(username,password,2)){
			...
			if (!temp.isEmpty()) {
				...
				request.setAttribute("error", "Unfortunately, this combination of username and password does not exist in our database.");
				// TODO - USERNAME AND PASSWORD DO NOT EXIST IN DB
			}
		}else{
			next ="/InstructorLogin.jsp";
			request.setAttribute("error", "Unfortunately, you haven't registered yet with this email.");
